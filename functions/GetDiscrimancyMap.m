function discrimancy = GetDiscrimancyMap(psd, band, window_frequency, frequencies)
    ...
    for n_electrode = 1:number_electrode
        ...
        ERD_ERS_both_feet = (10.*log10(PSD_both_feet) - 10.*log10(BaseLine_both_feet)) ./ (10.*log10(BaseLine_both_feet));
        ...
        ERD_ERS_both_hands = (10.*log10(PSD_both_hands) - 10.*log10(BaseLine_both_hands)) ./ (10.*log10(BaseLine_both_hands));
        
        for f = 1:size(PSD_both_feet,2)
            ERD_ERS_both_feet(:,f)
            ERD_ERS_both_hands(:,f)                        
            discrimancy(f, n_electrode) = abs(mean(PSD_both_feet(:,f)) - mean(PSD_both_hands(:,f))) / sqrt(std(PSD_both_feet(:,f))^2 + std(PSD_both_hands(:,f))^2);
        end
